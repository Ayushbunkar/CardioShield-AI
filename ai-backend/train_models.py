...
def train_models():
    ...
    xgb_search = RandomizedSearchCV(
        xgb_base, xgb_params, n_iter=40, cv=3, scoring='roc_auc',
    ...
    results['TabNet'] = print_metrics('GradientBoosting', y_test, gb_pred, gb_proba_test)
    ...
    results['NeuralNetwork'] = print_metrics('Neural Network', y_test, nn_pred, nn_proba_test)
    ...
    aucs = [results['XGBoost']['auc'], results['TabNet']['auc'], results['NeuralNetwork']['auc']]
    total_auc = sum(aucs)
    weights = [a / total_auc for a in aucs]
    print(f"    Weights: XGB={weights[0]:.3f}, GB={weights[1]:.3f}, NN={weights[2]:.3f}")
    
    ens_proba_test = weights[0] * xgb_proba_test + weights[1] * gb_proba_test + weights[2] * nn_proba_test
    ens_pred = (ens_proba_test >= 0.5).astype(int)
    results['StackedEnsemble'] = print_metrics('Weighted Ensemble', y_test, ens_pred, ens_proba_test)
    ...
    for name, m in results.items():
        print(f"  {name:<25} {m['accuracy']:>7.4f} {m['f1']:>7.4f} {m['precision']:>7.4f} {m['recall']:>7.4f} {m['auc']:>7.4f}")
